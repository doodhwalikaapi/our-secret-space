  ...
  const widget = cloudinary.createUploadWidget(
    ...
    (error, result) => {
      if (!error && result && result.event === 'success'){
        ...
        const pin = Math.random() > 0.5 ? 'heart' : 'daisy';

        const newItem = { url, pin, caption: '' };
        GALLERY_IMAGES = [...(GALLERY_IMAGES || []), newItem];
        renderGallery(GALLERY_IMAGES);

        showCopyModal(`{ url: "${url}", pin: "${pin}", caption: "" },`);
      }
    }
  );
...
/* ===========================================================
   COPY-URL MODAL — shown right after a successful upload so
   the person never needs to open dev tools.
   =========================================================== */
...
const copyModalCopyBtn = document.getElementById('copy-modal-copy');
const copyModalCloseBtn = document.getElementById('copy-modal-close');

function showCopyModal(snippet){
  copyModalText.value = snippet;
  copyModalNote.hidden = true;
...
copyModalCopyBtn.addEventListener('click', async () => {
  copyModalText.select();
  try{
    await navigator.clipboard.writeText(copyModalText.value);
  } catch(e){
    document.execCommand('copy');
  }
  copyModalNote.hidden = false;
});
copyModalCloseBtn.addEventListener('click', hideCopyModal);
copyModal.addEventListener('click', (e) => { if (e.target === copyModal) hideCopyModal(); });
